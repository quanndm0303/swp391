...
public class OutboundTransactionDetailValidator {
  ...
  public String checkPost(OutboundTransactionDetail outboundTransactionDetail) {
    ...
    if (id == null || !outboundTransactionRepository.existsById(
        outboundTransactionDetail.getTransaction_id())) {
    ...
    if (outboundTransactionDetail.getQuantity() <= 0 || String.valueOf(
        outboundTransactionDetail.getQuantity()).isBlank()) {
      return "Quantity is invalid";
    }
    Integer zone_id = outboundTransactionDetail.getZone_id();
    if (zone_id == null || !warehouseZoneRespository.existsById(
        outboundTransactionDetail.getZone_id())) {
